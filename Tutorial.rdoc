...
To example this Biogem's features we want to create a Rails application which provides a form to users. Data are submitted to TogoWS http://togows.dbcls.jp/ and the result is returned in a web page. In the mean time we want to store suers queries and return them in the page of TogoWS' result. We'll create a Rails application and a separated Biogem to store queries and provide connection to TogoWS, following "man in the middle" pattern.
    ...
    $ biogem create --with-engine Foo foo
    $ cd bioruby-foo
    $ rake version:write
    $ bundle install
...
== Have a plugin and want to add new features? Aka Actions

A project is never finished and many times we need to add new features or you just forgot to add an option in the command line. Lets say that you build a biogem but you forgot to add a database and expose the gem as a Rails Engine. You can still add  those functionalities later by simply add an `update` action instead of `create`

   $ biogem upgrade the-perfer-gem

Biogem will take care of adding the needed file and it will report the chances to you. The process is very similar to the one adopted by Rails. 
We currently support only `create` and `update`, in the future we want to implemente other actions like `destroy` or more fine grained modification/configuration to you base biogem.
...
Usage: biogem [actions] [options] reponame, e.g.

   $ biogem create the-perfect-gem

Valid actions are
        create                       initialize a brand new project
        upgrade                      upgrade the current project name with a new feature, support you want to extend your gem with and engine or a database  
...
These options are for Biogem
        ...
        --without-db                 do not create the database directory for a generic gem (used in conjunction with engine it create a web application similar to Rack, no db support)
