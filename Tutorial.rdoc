...
* [RVM]: It's optional but we suggest to use RVM, to install multiple ruby interpreters in your home directory; it's a very straightforward process
* GitHub & RubyGems: In order to install and use BioGem you must own a regular GitHub  http://github.com and  RubyGems http://rubygems.org account. Why ? Because for sharing and keeping track of source code we use github and for distributing the plugin we rely on RubyGems, the reference Ruby gem repository. To configure github and automate most of biogem's tasks:
...
The first step is to open Rakefile and modify the gem.summary and gem.description. These parameters are fundamentals because they describe your gem and its importance. If you let them as default the gem, as a countermeasure, will not be packed.
