...
* [RVM]: RVM is optional, but suggested. Not only can you specify the Ruby interpreter version that works best with Biogem, RVM also allows multiple ruby interpreters in your home directory for testing purposes. When using RVM, make sure to compile with openssl support, e.g. on Debian
...
===  Prerelease
To create a prerelease of your plugin, set the version to include a letter (e.g. 
alpha). This way the gem is registered as prerelease, listed on http://biogems.info/, but not listed in the standard gem lists.
